...
const RideList: React.FC<RideListProps> = ({ 
  ...
  const fetchRides = async (): Promise<void> => {
    try {
      ...
      const response = await rideTimelineServices.getDriverRides(driverId);
  ...
  const filteredRides = rides.filter(ride => {
    ...
    if (activeTab === "upcoming") {
      return ride.tripStatus === "YetToStart" || (tripStartDate > today && ride.tripStatus !== "Completed");
    } else {
      const isCompleted = ride.tripStatus === "Completed" || (tripStartDate < today && ride.tripStatus !== "YetToStart");
  ...
  const renderItem: ListRenderItem<Ride> = ({ item }) => {
    ...
    const fare = `₹${item.totalAmt.toFixed(2)}`;
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.locationRow}>
          ...
          <View>
            <TextNormal style={styles.locationText}>{item.tripStartLoc}</TextNormal>
        ...
        <View style={styles.locationRow}>
          ...
          <View>
            <TextNormal style={styles.locationText}>{item.tripEndLoc}</TextNormal>
        ...
        <View style={styles.footer}>
          <View style={styles.tripInfoContainer}>
            ...
            <TextNormal style={styles.tripInfoValue}>{item.tripType}</TextNormal>
          </View>
          <View style={styles.tripInfoContainer}>
            <TextNormal style={styles.tripInfoLabel}>Distance</TextNormal>
            <TextNormal style={styles.tripInfoValue}>{item.totalKm.toFixed(2)} km</TextNormal>
          </View>
          <View style={styles.tripInfoContainer}>
            <TextNormal style={styles.tripInfoLabel}>Invoice</TextNormal>
            <TextNormal style={styles.tripInfoValue}>{item.invoiceNo}</TextNormal>
          </View>
        </View>
      </View>
    );
